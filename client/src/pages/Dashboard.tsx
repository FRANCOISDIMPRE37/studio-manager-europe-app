  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="studio-card p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-600" style={{ color: 'var(--brand-text)', fontWeight: 600 }}>
            <Camera size={14} className="inline mr-2" style={{ color: 'var(--brand-cyan)' }} />
            Photos de traçabilité
          </h2>
          <div className="flex items-center gap-2">
            ...
            >
              <Camera size={12} /> Prendre une photo
            </button>
            {/* Upload depuis galerie */}
            <button
              onClick={() => photoInputRef.current?.click()}
              className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-all hover:opacity-90"
              style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--brand-text-muted)', border: '1px solid var(--brand-border)' }}
            >
              <Image size={12} /> Importer
            </button>
          </div>
        </div>

        {/* Inputs cachés */}
        <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={handlePhotoUpload} multiple />
        <input ref={photoInputRef} type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} multiple />

        {photos.length === 0 ? (
          <div className="text-center py-8">
            <Camera size={32} className="mx-auto mb-2 opacity-30" style={{ color: 'var(--brand-text-muted)' }} />
            <p className="text-sm" style={{ color: 'var(--brand-text-muted)' }}>Aucune photo de traçabilité</p>
            <p className="text-xs mt-1" style={{ color: 'var(--brand-text-muted)' }}>Prenez des photos de vos documents, bijoux, encres ou matériels</p>
          </div>
        ) : (
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
            {photos.map((photo: PhotoItem) => (
              <div key={photo.id} className="relative group rounded-lg overflow-hidden" style={{ aspectRatio: '1', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--brand-border)' }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 px-1.5 py-1" style={{ background: 'rgba(0,0,0,0.6)' }}>
                  <p className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.7)', fontSize: '10px' }}>{photo.date}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {selectedPhoto && (
          ...
          >
            <img src={selectedPhoto} alt="Photo agrandie" className="max-w-full max-h-full rounded-xl" style={{ maxHeight: '90vh' }} />
            <button
