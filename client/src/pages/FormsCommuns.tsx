...
function FormSection({ title }: { title: string }) {
  return (
    <div className="my-4">
      <div className="px-3 py-2 rounded-lg" data-print-color="section-blue" style={{
        ...printExactStyle,
        background: 'linear-gradient(135deg, #0a4a7a 0%, #0077aa 100%)',
        boxShadow: 'inset 0 0 0 9999px rgba(10,74,122,0.88)',
        color: '#ffffff',
        fontWeight: 700,
        fontFamily: 'Outfit',
        fontSize: '12px',
...
function FormField({
  label, value, onChange, placeholder, multiline, type, required, inputRef, onNext, autoCapitalize
}: {
  ...
  const isMissingRequired = !!required && trimmedValue === '';
  const isPhoneField = type === 'tel' || label.toLowerCase().includes('téléphone') || label.toLowerCase().includes('telephone');
  const requiredInputStyle = {
    background: isMissingRequired ? '#fff5f5' : '#f8f9fa',
    border: isMissingRequired ? '2px solid #b71c1c' : '1px solid #aaaaaa',
    color: '#111111',
    fontFamily: 'Outfit',
    minHeight: isPhoneField ? 44 : undefined,
    fontSize: isPhoneField ? 16 : undefined,
    WebkitAppearance: 'none' as const,
    boxShadow: isMissingRequired ? '0 0 0 3px rgba(183,28,28,0.14)' : 'none',
  };
  return (
    <div className="mb-3" data-required-wrapper={required ? 'true' : undefined}>
      <label className="block text-xs mb-1" style={{ color: isMissingRequired ? '#b71c1c' : '#111111', fontWeight: 800 }}>
        {label}{required && <span aria-hidden="true" style={{ color: '#b71c1c', fontWeight: 900 }}> *</span>}
      </label>
      {multiline ? (
        <textarea
          value={value}
          ...
          aria-invalid={isMissingRequired ? true : undefined}
        ...
        <input
          ...
          autoCapitalize={autoCapitalize || (label.toLowerCase().includes('nom') || label.toLowerCase().includes('prénom') || label.toLowerCase().includes('prenom') ? 'none' : 'sentences')}
          aria-required={required ? true : undefined}
          aria-invalid={isMissingRequired ? true : undefined}
      ...
      {isMissingRequired && (
        <div style={{ color: '#b71c1c', fontSize: 11, fontWeight: 800, marginTop: 4 }}>
...
function RadioField({ label, options, value, onChange, required }: {
  ...
  const isMissingRequired = !!required && !value;
  return (
    <div className="mb-3" data-required-wrapper={required ? 'true' : undefined}>
      {required && <input type="hidden" data-required="true" data-label={label} value={value} readOnly />}
      <label className="block mb-2" style={{ color: isMissingRequired ? '#b71c1c' : '#111111', fontWeight: 800, fontSize: '16px' }}>
        {label}{required && <span aria-hidden="true" style={{ color: '#b71c1c', marginLeft: 4, fontWeight: 900 }}>*</span>}
      </label>
      <div className="flex flex-wrap gap-2" role="radiogroup" aria-required={required ? true : undefined} aria-invalid={isMissingRequired ? true : undefined}>
        {(Array.isArray(options) ? options : []).map(opt => (
          <button
            ...
            style={{
              ...printExactStyle,
              minHeight: 44,
              background: value === opt ? '#0a4a7a' : (isMissingRequired ? '#fff5f5' : '#ffffff'),
              boxShadow: value === opt ? 'inset 0 0 0 9999px #0a4a7a' : (isMissingRequired ? '0 0 0 3px rgba(183,28,28,0.12), inset 0 0 0 9999px #fff5f5' : 'inset 0 0 0 9999px #ffffff'),
              border: `2px solid ${value === opt ? '#0a4a7a' : (isMissingRequired ? '#b71c1c' : '#555555')}`,
...
function DateSlashField({ label, value, onChange, required }: { label: string; value: string; onChange: (v: string) => void; required?: boolean }) {
  ...
  const isMissingRequired = !!required && (!dd || !mm || yyyy.length < 4);
  ...
  return (
    <div className="mb-3" data-required-wrapper={required ? 'true' : undefined}>
      ...
      >
        <input
          type="text" inputMode="numeric" autoComplete="off" maxLength={2} placeholder="JJ" value={dd}
          aria-required={required ? true : undefined}
          aria-invalid={isMissingRequired ? true : undefined}
          onChange={e => { const v = e.target.value.replace(/\D/g,''); update(v, mm, yyyy); if(v.length===2) refMM.current?.focus(); }}
          style={{ ...dateInputStyle, width: 34 }}
        />
        <span style={{ color: isMissingRequired ? '#b71c1c' : '#555555', fontWeight: 800 }}>/</span>
        <input
          ref={refMM} type="text" inputMode="numeric" autoComplete="off" maxLength={2} placeholder="MM" value={mm}
          aria-required={required ? true : undefined}
          aria-invalid={isMissingRequired ? true : undefined}
        ...
        <span style={{ color: isMissingRequired ? '#b71c1c' : '#555555', fontWeight: 800 }}>/</span>
        <input
          ref={refYYYY} type="text" inputMode="numeric" autoComplete="off" maxLength={4} placeholder="AAAA" value={yyyy}
          aria-required={required ? true : undefined}
          aria-invalid={isMissingRequired ? true : undefined}
          onChange={e => { const v = e.target.value.replace(/\D/g,''); update(dd, mm, v); }}
          onKeyDown={e => { if(e.key==='Backspace' && yyyy==='' && mm!=='') refMM.current?.focus(); }}
          style={{ ...dateInputStyle, width: 56 }}
        />
      </div>
      {isMissingRequired && (
        <div style={{ color: '#b71c1c', fontSize: 11, fontWeight: 800, marginTop: 4 }}>
...
function CheckboxField({ label, value, onToggle, warning, required }: {
  ...
  const isMissingRequired = !!required && !value;
  return (
    <div className="mb-2" data-required-wrapper={required ? 'true' : undefined}>
      <button
        type="button"
        onClick={onToggle}
        ...
        aria-invalid={isMissingRequired ? true : undefined}
        aria-pressed={value}
        style={{ ...printExactStyle, minHeight: 44, background: value ? 'rgba(10,74,122,0.05)' : (isMissingRequired ? '#fff5f5' : 'transparent'), border: isMissingRequired ? '2px solid #b71c1c' : '1px solid transparent', borderRadius: 8, boxShadow: isMissingRequired ? '0 0 0 3px rgba(183,28,28,0.12)' : 'none' }}
      >
        <div
          ...
          style={{
            ...
            boxShadow: value ? (warning ? 'inset 0 0 0 9999px rgba(255,152,0,0.2)' : 'inset 0 0 0 9999px rgba(10,74,122,0.2)') : (required ? 'inset 0 0 0 9999px #ffebee' : 'inset 0 0 0 9999px #ffffff'),
        ...
        <span className="text-sm" style={{ color: warning && value ? '#FF9800' : (isMissingRequired ? '#b71c1c' : '#111111'), fontFamily: 'Outfit', fontWeight: 700 }}>
          {label}{required && <span aria-hidden="true" style={{ color: '#b71c1c', marginLeft: 4, fontWeight: 900 }}>*</span>}
        </span>
      </button>
      {isMissingRequired && (
        <div style={{ color: '#b71c1c', fontSize: 11, fontWeight: 800, marginTop: 4, marginLeft: 8 }}>
          Obligatoire avant enregistrement
        </div>
      )}
    </div>
...
function LegalBox({ children, color = 'cyan' }: { children: React.ReactNode; color?: 'cyan' | 'orange' | 'green' | 'red' }) {
  ...
  return (
    <div className="p-3 rounded-xl mb-3 text-xs" data-print-color="legal-box" style={{ ...printExactStyle, background: c.bg, boxShadow: `inset 0 0 0 9999px ${c.bg}`, border: `1px solid ${c.border}`, color: c.text, fontFamily: 'Outfit', lineHeight: 1.6 }}>
      {children}
    </div>
  );
}
...
function RgpdMentions({ hideLegalIdCopyMention = false, printHidden = false }: { hideLegalIdCopyMention?: boolean; printHidden?: boolean } = {}) {
  const { state } = useApp();
  const email = state.salonInfo?.email || 'contact@votresalon.fr';
  return (
    <div className={`mt-4 mb-2 rounded-xl text-xs${printHidden ? ' no-print' : ''}`} data-print-color="rgpd-box" style={{
      ...printExactStyle,
      background: '#e8f5e9',
      boxShadow: 'inset 0 0 0 9999px #e8f5e9',
      ...
      <div style={{ marginBottom: 4 }}>
        ...
        3 ans minimum à compter de la majorité du mineur (Art. L1110-4 CSP).
      </div>
      <div style={{ marginBottom: 4 }}>
...
function WarningBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2 p-3 rounded-xl mb-3" data-print-color="warning-box" style={{ ...printExactStyle, background: '#ffcdd2', boxShadow: 'inset 0 0 0 9999px #ffcdd2', border: '2px solid #c62828' }}>
...
function AgeVerif({ dateNaissance }: { dateNaissance: string }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-1 px-2 py-1 rounded text-xs" data-print-color="age-box" style={{
      ...printExactStyle,
      background: isMajeur ? 'rgba(76,175,80,0.1)' : 'rgba(255,152,0,0.1)',
      boxShadow: isMajeur ? 'inset 0 0 0 9999px rgba(76,175,80,0.1)' : 'inset 0 0 0 9999px rgba(255,152,0,0.1)',
...
function PrintFooter({ salonInfo, docTitle, hideLegalIdCopyMention = false, hideRgpdBlock = false, hideDeveloperCredit = false }: {
  salonInfo: any;
  docTitle: string;
  hideLegalIdCopyMention?: boolean;
  hideRgpdBlock?: boolean;
      ...
      <div style={{
        ...
        {!hideRgpdBlock && (
          <>
            ...
            <div style={{ marginBottom: 4, lineHeight: 1.4 }}>
              ...
              3 ans minimum à compter de la majorité du mineur (Art. L1110-4 CSP).{!hideLegalIdCopyMention && <> Copie conservée par le salon — Pièces jointes : copie de la/des pièce(s) d’identité du/des représentant(s) légal/aux.</>}
            </div>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            {!hideDeveloperCredit && <div>Développé par Studio Manager Europe · www.studiomanagereurope.eu</div>}
          </div>
        </div>
      </div>
    </div>
