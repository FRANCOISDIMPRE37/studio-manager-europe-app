...
export default function Salaries() {
  ...
  const create = trpc.studioUsers.create.useMutation({
    onSuccess: () => { 
      utils.studioUsers.list.invalidate(); 
      setShowForm(false); 
      setForm({ prenom: '', nom: '', pin: '', role: 'employe' }); 
  ...
  const hasSignedEngagement = (salarieId: string): boolean => {
    // On cherche si un "client" (qui est en fait le salarié dans le système) a signé
    const salarieAsClient = state.clients.find(c => c.id === salarieId || (c.prenom === form.prenom && c.nom === form.nom));
    ...
    <div style={{ padding: 24, maxWidth: 600, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button 
          ...
          style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', color: 'white', borderRadius: 10, padding: '10px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}
        >
      ...
      {showForm && (
        <div style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 16, padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              <label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, display: 'block', marginBottom: 4 }}>Prénom *</label>
              <input style={inp} value={form.prenom} onChange={e => setForm(f => ({...f, prenom: e.target.value}))} />
            </div>
            <div>
              <label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, display: 'block', marginBottom: 4 }}>Nom *</label>
              <input style={inp} value={form.nom} onChange={e => setForm(f => ({...f, nom: e.target.value}))} />
            </div>
            <div>
              <label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, display: 'block', marginBottom: 4 }}>PIN 4 chiffres *</label>
              <input style={inp} maxLength={4} inputMode="numeric" value={form.pin} onChange={e => setForm(f => ({...f, pin: e.target.value.replace(/\D/g,'')}))} />
            </div>
            <div>
              <label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, display: 'block', marginBottom: 4 }}>Rôle</label>
        ...
        <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, marginBottom: 10 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
              {s.role} · PIN {s.hasPinSet ? '✅' : '❌'}
              {hasSignedEngagement(s.id) && <span className="ml-2 text-green-400">· RGPD ✅</span>}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            {!hasSignedEngagement(s.id) && (
              <button 
                ...
                style={{ background: 'rgba(131,208,245,0.15)', border: '1px solid rgba(131,208,245,0.3)', color: 'var(--brand-cyan)', borderRadius: 8, padding: '8px 12px', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
              >
                <FileText size={14} />
                Signer RGPD
              </button>
