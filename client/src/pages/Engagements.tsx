...
import { Shield, Trash2, CheckCircle, Clock, PenLine, Printer, FileText } from 'lucide-react';
...
export default function Engagements() {
  ...
  const fiche15Salarie = engagements[0]?.salarie || salaries[0];
  const fiche15Signed = engagements.length > 0;

  const openFiche15 = (salarieId?: string | number) => {
    navigate(salarieId ? `/rgpd-salarie?salarieId=${salarieId}` : '/rgpd-salarie');
  };

  const printFiche15 = (salarieId?: string | number) => {
    navigate(salarieId ? `/rgpd-salarie?salarieId=${salarieId}&print=1` : '/rgpd-salarie?print=1');
  };
  ...
  return (
    <div style={{ padding: 24, maxWidth: 720, margin: '0 auto' }}>
      ...
      {/* Carte document simple — Fiche 15 */}
      <div style={{ marginBottom: 24, padding: 16, borderRadius: 16, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
            <div style={{ width: 38, height: 38, borderRadius: 12, background: 'rgba(0,188,212,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <FileText size={18} color="#00bcd4" />
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, color: 'white', fontSize: 15, fontWeight: 700 }}>Fiche 15 — Engagement de Confidentialité</p>
              <p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,0.45)', fontSize: 12 }}>RGPD Art. 29 · Salariés</p>
            </div>
          </div>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, padding: '6px 10px', borderRadius: 20, background: fiche15Signed ? 'rgba(34,197,94,0.12)' : 'rgba(251,191,36,0.10)', color: fiche15Signed ? '#22c55e' : '#fbbf24', fontWeight: 600, whiteSpace: 'nowrap' }}>
            {fiche15Signed ? <CheckCircle size={11} /> : <Clock size={11} />}
            {fiche15Signed ? 'Rempli / signé' : 'En attente'}
          </span>
        </div>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          <button
            onClick={() => openFiche15(fiche15Salarie?.id)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '8px 12px', borderRadius: 9, background: 'rgba(99,102,241,0.15)', color: '#818cf8', fontWeight: 600, border: '1px solid rgba(99,102,241,0.3)', cursor: 'pointer', opacity: 1 }}
          >
            <FileText size={12} /> Ouvrir
          </button>
          <button
            onClick={() => printFiche15(fiche15Salarie?.id)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '8px 12px', borderRadius: 9, background: 'rgba(0,188,212,0.12)', color: '#00bcd4', fontWeight: 600, border: '1px solid rgba(0,188,212,0.28)', cursor: 'pointer', opacity: 1 }}
          >
              ...
              {engagements.map((e, i) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    ...
                    <button
                      onClick={() => openFiche15(e.salarieId)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '6px 12px', borderRadius: 9, background: 'rgba(99,102,241,0.15)', color: '#818cf8', fontWeight: 600, border: '1px solid rgba(99,102,241,0.3)', cursor: 'pointer' }}
                    >
                      <FileText size={11} />
                      Ouvrir
                    </button>
                    <button
                      onClick={() => printFiche15(e.salarieId)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '6px 12px', borderRadius: 9, background: 'rgba(0,188,212,0.12)', color: '#00bcd4', fontWeight: 600, border: '1px solid rgba(0,188,212,0.28)', cursor: 'pointer' }}
                    >
                      <Printer size={11} />
                      Imprimer
                    </button>
            ...
            <div>
              ...
              {enAttente.map((s: Client) => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <button
                      onClick={() => printFiche15(s.id)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '6px 12px', borderRadius: 9, background: 'rgba(0,188,212,0.12)', color: '#00bcd4', fontWeight: 600, border: '1px solid rgba(0,188,212,0.28)', cursor: 'pointer' }}
                    >
                      <Printer size={11} />
                      Imprimer
                    </button>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, padding: '6px 10px', borderRadius: 20, background: 'rgba(251,191,36,0.10)', color: '#fbbf24', fontWeight: 600 }}>
                      <Clock size={11} />
                      En attente
                    </span>
                    <button
                      onClick={() => openFiche15(s.id)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '6px 12px', borderRadius: 9, background: 'rgba(99,102,241,0.15)', color: '#818cf8', fontWeight: 600, border: '1px solid rgba(99,102,241,0.3)', cursor: 'pointer' }}
                    >
                      <PenLine size={11} />
                      Signer
